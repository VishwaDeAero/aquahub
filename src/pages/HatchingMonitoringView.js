...
function HatchingMonitoringView() {
    ...
    const [hatchingData, setHatchingData] = useState([
        { id: 1, date: "11.09.2023", naupilliStage: "01", numberEggs: "222", broodstock: "Broodstock A" },
        { id: 2, date: "12.09.2023", naupilliStage: "02", numberEggs: "333", broodstock: "Broodstock B" },
        { id: 3, date: "13.09.2023", naupilliStage: "03", numberEggs: "444", broodstock: "Broodstock C" },
    ]);
    ...
    // Confirm Delete
    const confirmDelete = () => {
        setHatchingData(hatchingData.filter((item) => item.id !== selectedHatching));
        setIsDeleteModalOpen(false);
    };

    // Save (for both Create & Edit)
    const handleSave = (updatedHatching) => {
        if (selectedHatching) {
            // Update existing
            setHatchingData((prev) =>
                prev.map((item) => (item.id === updatedHatching.id ? updatedHatching : item))
            );
        } else {
            // Add new
            setHatchingData((prev) => [...prev, { id: prev.length + 1, ...updatedHatching }]);
        }
        ...
        <AppLayout title="Hatching Monitoring">
            ...
            <div className="p-6 border rounded-md bg-white shadow-sm">
                ...
                <FormModal
                    isOpen={isModalOpen}
                    title="Edit Hatching"
                    form={<HatchingMonitoringForm initialData={selectedHatching} onSubmit={handleSave} onCancel={() => setIsModalOpen(false)} />}
